...
const RADAR_KEYWORDS = [
    ...
    "新歌", "热歌", "抖音热歌", "网络歌曲", "影视原声", "游戏音乐",
    "纯音乐", "钢琴曲", "吉他", "小提琴", "治愈系", "励志", "爱情",
    "伤感", "快乐", "放松", "工作学习", "运动健身", "开车", "旅行"
];
...
async function checkAndAutoAddRadarSongs() {
    ...
    if (remainingSongs <= 3) {
        ...
        try {
            ...
            debugLog(`自动添加雷达歌曲，使用关键词: ${randomKeyword}`);

            // 使用酷我音源进行搜索
            const searchResults = await API.search(randomKeyword, "kuwo", 50, 1);
            ...
            if (searchResults.length > 0) {
                ...
                if (songsToAdd.length > 0) {
                    ...
                    debugLog(`自动添加雷达歌曲成功: 添加 ${songsToAdd.length} 首歌曲 (关键词: ${randomKeyword})`);
...
async function exploreOnlineMusic() {
    ...
    try {
        ...
        // 从备选关键词中随机选择一个
        const randomKeyword = RADAR_KEYWORDS[Math.floor(Math.random() * RADAR_KEYWORDS.length)];
        debugLog(`探索雷达使用随机关键词: ${randomKeyword}`);
        ...
        if (searchResults.length > 0) {
            ...
            if (songsToAdd.length > 0) {
                ...
                debugLog(`探索雷达成功: 关键词 "${randomKeyword}", 添加 ${songsToAdd.length} 首歌曲`);
