...
function setDocumentGradient(gradient, {
	immediate = false
} = {}) {
...
function scheduleDeferredPaletteUpdate(imageUrl, options = {}) {
	...
	if (typeof window.requestIdleCallback === "function") {
		...
		deferredPaletteHandle = window.requestIdleCallback(runner, {
			timeout: 800
		});
	} else {
...
function setupInteractions() {
	...
	// 定义 initializePlaylistEventHandlers 函数
			...
			dom.playPauseBtn.addEventListener("click", togglePlayPause);
				...
				if (state.playlistSongs.length > 0) {
							...
							API.fetchJson(picUrl)
								.then(data => {
									...
									img.onload = () => {
										...
										scheduleDeferredPaletteUpdate(imageUrl, {
					...
					function displaySearchResults(newItems, options = {}) {
						...
						const {
							reset = false, totalCount = state.searchResults.length
						} = options;
						...
						reader.onload = () => {
							try {
								...
								const {
									added,
									duplicates
								} = handleImportedPlaylistItems(items);
								if (added > 0) {
									const duplicateHint = duplicates > 0 ? `，${duplicates} 首已存在` : "";
									showNotification(`成功导入 ${added} 首歌曲${duplicateHint}`, "success");
								} else {
            ...
            <button class="playlist-item-download" type="button" data-playlist-action="download" data-index="${index}" title="下载">
                <i class="fas fa-download"></i>
					...
					async function playSong(song, options = {}) {
						const {
							autoplay = true, startTime = 0, preserveProgress = false
						} = options;
